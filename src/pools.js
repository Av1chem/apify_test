...
exports.sGenPool = new Apify.AutoscaledPool({
  ...
  runTaskFunction: async () => {
    ...
    } = globals.sGenQueue.pop();
    let promises = generateSearchRequests(
      fromRequest,
      response,
      requestQueue,
      isTestMode,
...
exports.dGenPool = new Apify.AutoscaledPool({
  ...
  loggingIntervalSecs: null,
  runTaskFunction: async () => {
    let { fromRequest, response, requestQueue, mappingDataset } =
      globals.dGenQueue.pop();
    let promises = generateDetailRequests(fromRequest, response, requestQueue);

    await Promise.all([
      promises,
      mappingDataset.pushData(
        response.providers.map((x) =>
          Object.assign(x, { network: fromRequest.userData.network })
        )
      ),
    ]);
  },
